...
# ============================================================================
# Phase 1b — Upwind SBP operator pair  D_+  /  D_-
#
# Based on Mattsson (2017) "Diagonal-norm upwind SBP operators".
# Key identities (hold in H-inner product):
#   D_c = (D_+ + D_-) / 2  ==  sbp_dx  (central operator)
#   Σ   = (D_- - D_+) / 2  ≥  0        (upwind dissipation, semi-definite)
#
# Boundary rows are IDENTICAL to sbp_dx for the same order — only interior
# stencils differ.
#
# Interior coefficients (offsets from centre):
#   Order 2
#     D_-[i]  = (u[i] - u[i-1]) / dx          (offsets  0, -1)
#     D_+[i]  = (u[i+1] - u[i]) / dx          (offsets +1,  0)
#     Σ[i]    = (u[i-1] - 2u[i] + u[i+1])/(2*dx)
#
#   Order 4   (Mattsson 2017, p=2; interior accuracy 3rd-order)
#     D_-[i]  = ( u[i-2]/6 - u[i-1] + u[i]/2 + u[i+1]/3 ) / dx
#     D_+[i]  = ( -u[i-1]/3 - u[i]/2 + u[i+1] - u[i+2]/6 ) / dx
#     Σ[i]    = (u[i-2] - 4u[i-1] + 6u[i] - 4u[i+1] + u[i+2]) / (12*dx)
#
# ============================================================================
...
def sbp_dx_pm(dp, dm, u, nx, dx, order):
    """
    Compute D_+ u (dp) and D_- u (dm) in-place using upwind SBP operators.

    Boundary stencils are identical to sbp_dx for the same order.
    Only interior stencils differ (upwind-biased, order 2 or 4).

    Parameters
    ----------
    dp, dm : (nx,) float64 arrays, output
    u      : (nx,) float64 array, input
    nx     : int, number of grid points
    dx     : float, grid spacing
    order  : int, SBP order (2 or 4; 6 not supported for upwind)
    """
    m      = nx - 1
    inv_dx = 1.0 / dx

    if order == 2:
        # ── Boundary rows: identical to central sbp_dx order 2 ──────────────
        dp[0] = (u[1] - u[0]) * inv_dx
        dm[0] = dp[0]
        dp[m] = (u[m] - u[m - 1]) * inv_dx
        dm[m] = dp[m]
        # ── Interior: forward / backward first differences ───────────────────
        for i in prange(1, m):
            dm[i] = (u[i]     - u[i - 1]) * inv_dx   # D_-: backward
            dp[i] = (u[i + 1] - u[i])     * inv_dx   # D_+: forward

    else:  # order == 4
        # ── Left boundary rows: same as sbp_dx order 4 ───────────────────────
        _v0 = (-24.0/17.0*u[0] + 59.0/34.0*u[1]
               - 4.0/17.0*u[2] - 3.0/34.0*u[3]) * inv_dx
        _v1 = (-0.5*u[0] + 0.5*u[2]) * inv_dx
        _v2 = (4.0/43.0*u[0] - 59.0/86.0*u[1]
               + 59.0/86.0*u[3] - 4.0/43.0*u[4]) * inv_dx
        _v3 = (3.0/98.0*u[0] - 59.0/98.0*u[2]
               + 32.0/49.0*u[4] - 4.0/49.0*u[5]) * inv_dx
        dp[0] = dm[0] = _v0
        dp[1] = dm[1] = _v1
        dp[2] = dm[2] = _v2
        dp[3] = dm[3] = _v3
        # ── Right boundary rows: same as sbp_dx order 4 ──────────────────────
        _vm   = ( 24.0/17.0*u[m]   - 59.0/34.0*u[m-1]
                  + 4.0/17.0*u[m-2]  + 3.0/34.0*u[m-3]) * inv_dx
        _vm1  = (0.5*u[m] - 0.5*u[m-2]) * inv_dx
        _vm2  = (-4.0/43.0*u[m]   + 59.0/86.0*u[m-1]
                  - 59.0/86.0*u[m-3] + 4.0/43.0*u[m-4]) * inv_dx
        _vm3  = (-3.0/98.0*u[m]   + 59.0/98.0*u[m-2]
                  - 32.0/49.0*u[m-4]  + 4.0/49.0*u[m-5]) * inv_dx
        dp[m]     = dm[m]     = _vm
        dp[m - 1] = dm[m - 1] = _vm1
        dp[m - 2] = dm[m - 2] = _vm2
        dp[m - 3] = dm[m - 3] = _vm3
        # ── Interior: Mattsson (2017) order-4 upwind pair ────────────────────
        # D_-[i] = ( 1/6 u[i-2]  -  u[i-1]  + 1/2 u[i]  + 1/3 u[i+1] ) / dx
        # D_+[i] = (-1/3 u[i-1]  - 1/2 u[i] +     u[i+1] - 1/6 u[i+2] ) / dx
        for i in prange(4, m - 3):
            dm[i] = (0.166666666666667*u[i - 2] - u[i - 1]
                     + 0.5*u[i] + 0.333333333333333*u[i + 1]) * inv_dx
            dp[i] = (-0.333333333333333*u[i - 1] - 0.5*u[i]
                     + u[i + 1] - 0.166666666666667*u[i + 2]) * inv_dx


@njit(parallel=True, cache=True)
def sbp_dx_sigma(sigma_u, u, nx, dx, order):
    """
    Compute the upwind dissipation Σ u = (D_- u − D_+ u) / 2 in-place.

    Boundary rows are set to zero (SAT handles boundary stability).

    Parameters
    ----------
    sigma_u : (nx,) float64 array, output
    u       : (nx,) float64 array, input
    nx      : int, number of grid points
    dx      : float, grid spacing
    order   : int, SBP order (2 or 4)
    """
    m      = nx - 1
    inv_dx = 1.0 / dx

    if order == 2:
        # Boundary rows: Σ = 0 (D_- == D_+ at boundaries)
        sigma_u[0] = 0.0
        sigma_u[m] = 0.0
        # Interior: Σ u[i] = (-u[i-1] + 2u[i] - u[i+1]) / (2 dx)
        # = (D_- − D_+)/2 = backward−forward.  Semi-definite: u^T H Σ u = ½Σ(u[i]−u[i-1])² ≥ 0
        for i in prange(1, m):
            sigma_u[i] = 0.5 * (-u[i - 1] + 2.0*u[i] - u[i + 1]) * inv_dx

    else:  # order == 4
        # Boundary rows: Σ = 0 at first/last 4 points
        sigma_u[0] = sigma_u[1] = sigma_u[2] = sigma_u[3] = 0.0
        sigma_u[m] = sigma_u[m-1] = sigma_u[m-2] = sigma_u[m-3] = 0.0
        # Interior: Σ u[i] = (u[i-2] − 4u[i-1] + 6u[i] − 4u[i+1] + u[i+2])/(12 dx)
        c = inv_dx / 12.0
        for i in prange(4, m - 3):
            sigma_u[i] = c * (u[i - 2] - 4.0*u[i - 1] + 6.0*u[i]
                              - 4.0*u[i + 1] + u[i + 2])
...
# ============================================================================
# Phase 2b — Upwind elastic RHS  _elastic_rate_upwind
#
# RHS equations (Mattsson 2017, characteristic split):
#   ρ v̇ = D_c s + ρ cs Σ v   (upwind adds velocity dissipation)
#     ṡ = μ D_c v + cs Σ s    (upwind adds stress  dissipation)
# where D_c = sbp_dx, Σ = sbp_dx_sigma, cs = sqrt(mu/rho).
#
# SAT boundary/interface penalties are identical to _elastic_rate — they
# act on the same characteristic variables and do not interact with the
# interior upwind correction.
# PML: the d(x) damping term is applied to the full state, as in the
# central case.
# ============================================================================
...
def _elastic_rate_upwind(hv_l, hs_l, v_l, s_l,
                         ...
                         friction_params):
    """
    Upwind-SBP elastic RHS.  Identical signature and semantics to
    _elastic_rate, but adds Mattsson (2017) upwind interior dissipation.

    Parameters
    ----------
    Same as _elastic_rate.  order must be 2 or 4.
    ...
    # Central spatial derivatives (D_c == sbp_dx)
    vx_l = np.empty(nx); sx_l = np.empty(nx)
    vx_r = np.empty(nx); sx_r = np.empty(nx)
    sbp_dx(vx_l, v_l, nx, dx, order)
    sbp_dx(sx_l, s_l, nx, dx, order)
    sbp_dx(vx_r, v_r, nx, dx, order)
    sbp_dx(sx_r, s_r, nx, dx, order)

    # Upwind dissipation: Σ v and Σ s
    sigma_vl = np.empty(nx); sigma_sl = np.empty(nx)
    sigma_vr = np.empty(nx); sigma_sr = np.empty(nx)
    sbp_dx_sigma(sigma_vl, v_l, nx, dx, order)
    sbp_dx_sigma(sigma_sl, s_l, nx, dx, order)
    sbp_dx_sigma(sigma_vr, v_r, nx, dx, order)
    sbp_dx_sigma(sigma_sr, s_r, nx, dx, order)

    for i in range(nx):
        hv_l[i] = inv_rho * sx_l[i] + cs * sigma_vl[i] - d_l[i] * v_l[i]
        hs_l[i] = mu * vx_l[i]      + cs * sigma_sl[i] - d_l[i] * s_l[i]
        hv_r[i] = inv_rho * sx_r[i] + cs * sigma_vr[i] - d_r[i] * v_r[i]
        hs_r[i] = mu * vx_r[i]      + cs * sigma_sr[i] - d_r[i] * s_r[i]
...
# ============================================================================
# Phase 3b — Upwind anelastic RHS  _anelastic_rates_upwind
#
# Modifies _anelastic_rates to add Mattsson (2017) upwind dissipation while
# preserving the GSLS augmented-energy proof.
#
# Key rule (see plan_upwind.md §"Extension to Anelastic"):
#   • ṡ = μ D_c v + cs Σ s − Σ_l η_l    (D_c v drives memory ODEs; Σ s adds dissipation)
#   • v̇ = (1/ρ) D_c s + cs Σ v           (Σ v adds velocity dissipation)
#   • η̇_l = [w_l μ Q_s^{-1} D_c v − η_l] / τ_l   (uses D_c v, NOT Σ v)
#
# The η ODE forcing uses the CENTRAL derivative `vx` so that the cross-
# cancellation term  η^T H (D_c v)  in the augmented energy estimate still
# cancels against −Σ_l η_l in ṡ.  The upwind correction Σ s contributes
# only additional dissipation and does NOT disturb this cancellation.
#
# cs is computed point-by-point: cs_i = sqrt(mu_arr[i]/rho_arr[i]).
# ============================================================================

@njit(cache=True)
def _anelastic_rates_upwind(hv_l, hs_l, Deta_l,
                             hv_r, hs_r, Deta_r,
                             v_l, s_l, eta_l,
                             v_r, s_r, eta_r,
                             slip, psi,
                             ...
                             friction_params):
    """
    Upwind-SBP anelastic RHS.  Identical signature to _anelastic_rates;
    adds Mattsson (2017) upwind dissipation without breaking the GSLS
    augmented-energy stability proof.

    Parameters
    ----------
    Same as _anelastic_rates.  order must be 2 or 4.
    ...
    # ── Central derivatives D_c v, D_c s (used for elastic rate + eta ODE) ──
    vx_l = np.empty(nx);  sx_l = np.empty(nx)
    vx_r = np.empty(nx);  sx_r = np.empty(nx)
    sbp_dx(vx_l, v_l, nx, dx, order)
    sbp_dx(sx_l, s_l, nx, dx, order)
    ...
    # ── Elastic + PML + upwind rates (point-by-point variable material) ──────
    for i in range(nx):
        cs_i    = math.sqrt(mu_arr[i] / rho_arr[i])
        hv_l[i] = sx_l[i] / rho_arr[i]  + cs_i * sigma_vl[i] - d_l[i] * v_l[i]
        hs_l[i] = mu_arr[i] * vx_l[i]   + cs_i * sigma_sl[i] - d_l[i] * s_l[i]
        hv_r[i] = sx_r[i] / rho_arr[i]  + cs_i * sigma_vr[i] - d_r[i] * v_r[i]
        hs_r[i] = mu_arr[i] * vx_r[i]   + cs_i * sigma_sr[i] - d_r[i] * s_r[i]

    # ── Anelastic memory-variable contribution: uses D_c v (vx_l), NOT Σ v ──
    _anelastic_rhs(hs_l, Deta_l, eta_l, vx_l, mu_arr, Qs_inv, tau, weight, nx)
    _anelastic_rhs(hs_r, Deta_r, eta_r, vx_r, mu_arr, Qs_inv, tau, weight, nx)
