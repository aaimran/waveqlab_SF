...
# Per-stencil, per-order CFL stability limits for classical RK4.
# Central: from RK4 imaginary-axis bound vs SBP spectral radius.
# Upwind / Upwind-DRP: tighter due to added dissipation eigenvalues
#   (order 6 upwind not yet implemented; order 2 upwind_drp not applicable).
_CFL_MAX = {
    'central':    {2: 1.00, 4: 0.68, 6: 0.45},
    'upwind':     {2: 0.85, 4: 0.60},
    'upwind_drp': {4: 0.45},
}
...
def parse_infile(path):
    ...
    _str_keys = {'fric_law', 'output_prefix', 'response',
                 'rho_file', 'cs_file', 'Qs_inv_file', 'stencil'}
...
def build_params(user):
    ...
    p['stencil']  = str(p.get('stencil', 'central')).lower()
...
def validate(p):
    ...
    if p['stencil'] not in VALID_STENCILS:
        errors.append(f"stencil='{p['stencil']}' is not valid. Choose from: {VALID_STENCILS}")
    else:
        stencil = p['stencil']
        order   = int(p['order'])
        if stencil in ('upwind', 'upwind_drp') and order not in (2, 4):
            errors.append(
                f"stencil='{stencil}' is only implemented for order 2 and 4 "
                f"(got order={order}).")
        if stencil == 'upwind_drp' and order != 4:
            errors.append(
                f"stencil='upwind_drp' requires order=4 (11-pt DRP interior); "
                f"got order={order}.")
        if stencil == 'upwind_drp' and int(p.get('nx', 501)) < 25:
            errors.append(
                f"stencil='upwind_drp' requires nx >= 25 for the 11-pt interior stencil.")

    # Caveat 3: enforce stencil- and order-dependent CFL stability limit
    if not (0 < float(p['cfl']) <= 1.0):
        errors.append(f"cfl={p['cfl']} must be in (0, 1]")
    else:
        stencil_cfl = _CFL_MAX.get(p.get('stencil', 'central'), _CFL_MAX['central'])
        cfl_max     = stencil_cfl.get(int(p.get('order', 6)), 1.0)
        if float(p['cfl']) > cfl_max:
            errors.append(
                f"cfl={p['cfl']} exceeds the RK4+SBP stability limit "
                f"{cfl_max} for stencil='{p['stencil']}', order={p['order']}. "
                f"Reduce cfl or switch to a lower-order / central scheme.")
