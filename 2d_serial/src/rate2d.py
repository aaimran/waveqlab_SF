def elastic_rate2d(D_l, F_l, Mat_l, X_l, Y_l, t, nf, nx, ny, dx, dy, order, r_l, source_parameter, D_r, F_r, Mat_r, X_r, Y_r, r_r, friction_parameters, slip, psi, dslip, dpsi,fric_law, FaultOutput0, stage, Y0):
    ...
    Y = Y_r[0, :]
    interfacecondition2d(F_l, F_r, Fhat_l, Fhat_r, Mat_l, Mat_r, nx, ny, friction_parameters, slip, psi, dslip, dpsi, fric_law, FaultOutput0, Y, t, stage, Y0)
    
    # impose boundary conditions using penalty: SAT
    elastic_impose_bc(D_l, F_l, Fhat_l, Mat_l, nx, ny, nf, dx, dy, order, r_l,'left')
    elastic_impose_bc(D_r, F_r, Fhat_r, Mat_r, nx, ny, nf, dx, dy, order, r_r,'right')

    

    
def interfacecondition2d(F_l, F_r, Fhat_l, Fhat_r, Mat_l, Mat_r, nx, ny, friction_parameters, slip, psi, dslip, dpsi, fric_law, FaultOutput0, Y, t, stage, Y0): 
    import numpy as np
    ...
    for j in range(0, ny):
        ...
        #friction_parameters[8][j] = sigma_n
        
        #print(sigma_n)
        
        #return_int = interfacedata.interface_condition(vy_l, Ty_l, vy_r, Ty_r,rho_l,mu_l,rho_r,mu_r)
        
        #friction_parameters_index = [friction_parameters[0][j] , friction_parameters[1][j] ,friction_parameters[2][j] ,friction_parameters[3][j] ,friction_parameters[4][j] ,friction_parameters[5][j] ,friction_parameters[6][j] ,friction_parameters[7][j] ,sigma_n ,friction_parameters[9][j] ,friction_parameters[10][j] , friction_parameters[11][j]]
        ...
        return_int = interfacedata.friction_law(vy_l, Ty_l, vy_r, Ty_r, float(slip[j, 0]), float(psi[j, 0]), rho_l, mu_l, rho_r, mu_r, friction_parameters_index, fric_law)
        vy_m = return_int['V_m']
        vy_p = return_int['V_p']
        Ty_m = return_int['T_m']
        Ty_p = return_int['T_p']
        ...
        if stage == 0:
            ...
            FaultOutput0[j, 3] = Ty_m+friction_parameters[3,j]
...
def elastic_impose_bc(D, F, Fhat, Mat, nx, ny, nf, dx, dy, order, r, side):
    ...
    if side == 'left' :
        ...
        interface.Interface_Fault(BFnx, F, Fhat, Mat, nx, ny, side)
        
    if side == 'right' : 
        boundarycondition.bcp2dx(BFnx, F, Mat, nx, ny, r,'elastic')
        interface.Interface_Fault(BF0x, F, Fhat, Mat, nx, ny, side)
